...
│   │   ├── users/                # 用户/角色/部门管理
│   │   │   ├── api/
│   │   │   │   ├── users.api.ts      # Axios 请求函数（调 shared/api/generated）
│   │   │   │   ├── users.queries.ts  # query key 工厂 + queryOptions（TanStack Query）
│   │   │   │   └── users.types.ts    # 该模块本地类型（表单态等，API 类型来自 generated）
...
│   │   └── generated/            # OpenAPI codegen 产物（自动生成，禁手改）
│   │       ├── types.ts          # 后端 schema 生成的类型
│   │       └── services.ts       # 后端生成的请求函数
│   │
│   ├── locales/                  # i18n 资源
│   │   ├── zh-CN/
...
```
api/generated/         ← OpenAPI codegen（后端类型 + 请求函数，禁手改）
   ↓
features/<m>/api/*.api.ts    ← 业务请求函数，调 generated，是人写的边界
   ↓
features/<m>/api/*.queries.ts ← queryOptions + key 工厂
```

请求统一经 `api/client.ts` 的 Axios 实例；拦截器挂 token、解析 `problem+json` 的 `code` → 查 i18n → toast/表单级提示。
