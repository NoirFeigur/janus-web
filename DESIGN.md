...
- Debounced keyword input (`w-64`) + employee-no input (`w-40`) grouped with `gap-2` as the search cluster (`useDebouncedValue` → `params`); status `Segmented` control (point-select, no debounce) sits `gap-3` apart as the view filter. Inputs sit left; count / reset / actions sit right (`ml-auto`). Both inputs support `onPressEnter` → **flush** (a submit nonce forces `params` to recompute from raw filter values immediately, skipping the debounce wait) — the fast path for users who type-then-enter.
- A **live result count** (`pages.user.totalCount`, tabular-nums, `aria-live="polite"`) is **always rendered** as a **recessed-fill pill** (`bg-surface-sunken rounded-md px-2.5 py-1`, secondary ink) — an independent info block that reads the result-set size at a glance. It is the toolbar's job, so the pagination bar drops its `showTotal` to avoid a duplicate total.
...
- **Pagination bar**: separated from the table body by a `1px` top border (§5 flat-border division), `12px 16px` padding. `pageSizeOptions [10, 20, 50, 100]` + `showQuickJumper` for jumping page-size tiers and deep pages without paging one at a time. `showTotal` stays off (the toolbar count owns the total).
