...
### Scrollbar (global)
- **Intent**: thin, **rounded**, **no up/down arrow buttons**, **transparent at rest** — quiet by default, revealed on hover/scroll, never the heavy always-on grey OS bar. This intent is delivered by **two mechanisms** split by surface, because pure CSS can't cover both.
- **Simple containers** (page root, drawers, dropdowns — no hovered child elements) → **`::-webkit-scrollbar` hover-reveal**: thin (`10px`), transparent track, thumb transparent at rest, appearing only when the container is hovered or focus-within (`--brand-scrollbar-thumb`, `rgba(48,58,84,.22)`), deepening on the thumb itself (`--brand-scrollbar-thumb-hover`, `.34`). `::-webkit-scrollbar-button { display: none }` removes the arrows. Firefox falls back to the standard `scrollbar-width`/`scrollbar-color`, quarantined behind `@supports not selector(::-webkit-scrollbar)` so it never collides with the webkit path in Chromium.
- **Tables** (`.ant-table-body`) → **OverlayScrollbars** (`overlayscrollbars@2`, mounted non-destructively via [`useTableOverlayScrollbar`](file:///D:/workspace/janus-web/src/hooks/useTableOverlayScrollbar.ts)). Why the split: table rows (`tr.ant-table-row`) carry their **own** hover, and Blink's decade-old defect ([WebKit #109230](https://bugs.chromium.org/p/chromium/issues/detail?id=109230): a descendant's hover doesn't repaint an ancestor's scrollbar) makes CSS hover-reveal on tables unreliable — the thumb flickers/vanishes the moment the pointer enters a row. Chromium 114 (2023-06) also removed the overlay-scrollbar capability, so a *true* overlay hover-reveal is no longer reachable in pure CSS. OverlayScrollbars paints its own overlay thumb and drives show/hide off its **own pointer tracking** (`autoHide: 'leave'`, VS Code style), structurally immune to the Blink defect; the overlay occupies no layout channel and ships no arrows.
- **Overlay gutter**: an overlay thumb floats *over* content, so the horizontal bar would otherwise ride on the last row's bottom edge (measured 10px overlap). The hook's `updated` callback reads `state().hasOverflow.x` and adds a `10px` bottom padding to `.ant-table-body` **only when the table actually overflows horizontally** — pushing the bar into its own space below the last row, with **no wasted whitespace** when there's no bar.
- **OverlayScrollbars theming**: the `.os-theme-janus` theme lives in the **non-layered** tail of `global.css` — OverlayScrollbars' base CSS is itself non-layered and outranks any `@layer`, so a themed `--os-handle-bg` placed inside `@layer components` was silently overridden (thumb rendered height-0/transparent). Three handle tints map to `--brand-scrollbar-thumb` / `-hover` / `-active` (`.22` / `.34` / `.46`; the resting tint is stronger than the webkit path because OverlayScrollbars only surfaces on hover/scroll and needs more presence when it does appear).
- **Tokens**: `scrollbarThumb` / `scrollbarThumbHover` / `scrollbarThumbActive` flow through the same primitive→semantic→`brandCssVars`→codegen pipeline as every other color; no literals in CSS. The TagsView strip opts out entirely (`scrollbar-width: none`) — it scrolls via wheel/drag, no visible bar.
- **Note on verification**: Playwright/headless runs Linux Chromium (native scrollbars render differently from Windows Chrome), `getComputedStyle` echoes authored pseudo-element CSS even when the engine ignores it, and synthetic pointer events don't drive OverlayScrollbars' real hardware-pointer tracking. Automated QA can confirm **DOM structure / cascade / bar geometry** (mount, theme, gutter, no-overflow suppression — all verified), but **not** the final Windows rendering or the pointer-driven show/hide timing. **Windows Chrome is the source of truth for this surface.**
...
- **Table fills the card's remaining height** (`.janus-table-fill` on the scroll region, styled in `global.css`): AntD's table height is content-driven — with few rows it would collapse to a short block, stranding the pagination mid-card above a large empty band. The fix makes the whole AntD wrapper chain (`.ant-pro-table` → `.ant-pro-card` → `-body` → `.ant-table-wrapper` → `.ant-spin-nested-loading` → `.ant-spin-container`) a flex column; the top `.ant-pro-table` anchors to the scroll region with `min-height: 100%`, and each level below uses `flex: 1` to stretch inside its flex parent — a percentage `min-height` chain breaks at the second level because a height *derived from* `min-height` is not a definite height for a child's percentage. The terminal `.ant-table` takes `flex: 1` to absorb the slack and push `.ant-pagination` to the card bottom. With few rows the table fills the card and the pagination pins to the bottom; with many rows the chain overflows and the scroll region scrolls (sticky header intact) — both without a fixed height.
...
### Columns
- index (`#`) / username / employee-no / real-name / email / status / created-at / actions.
- **Index column** (`#`, `width: 64`, centered): a **continuous** row number across pages — `(current - 1) * pageSize + index + 1`, page/pageSize tracked in a ref refreshed in `request` — not a per-page 1…N reset. Rendered tertiary-toned + tabular-nums as weak meta-info, never competing with content.
- Null or empty cells render a tertiary-toned `—` dash-placeholder — never a blank cell.
- **username** at `font-weight 500` as a scan anchor — applied via `onCell` (body `td` only), **not** column `className`: a column `className` lands on the header `th` too and would drag that one header down to 500, making it read *lighter* than its `font-weight 600` siblings. Header weight is owned solely by the global `thead th` rule.
