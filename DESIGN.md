...
### Scrollbar (global)
- **Look**: thin (`10px`), **rounded**, **no up/down arrow buttons**, transparent track/gutter (the reserved channel shows the container background, no grey rail).
- **Hover-reveal**: the thumb is **transparent at rest** and only appears when its scroll container is hovered or focus-within (`--brand-scrollbar-thumb`, `rgba(48,58,84,.10)`), deepening slightly when the pointer is on the thumb itself (`--brand-scrollbar-thumb-hover`, `.15`). Quiet by default, never the heavy always-on grey OS bar.
- **Chromium mutual-exclusivity trap** (the crux): in Chromium, the standard `scrollbar-width` / `scrollbar-color` and the legacy `::-webkit-scrollbar` pseudo-elements are **mutually exclusive** — any non-`auto` standard property makes Chrome **ignore all `::-webkit-scrollbar` rules** and fall back to the native OS scrollbar (on Windows: an always-on grey bar *with* arrow buttons). Two sources triggered this: (1) the global fallback — the standard properties are quarantined behind `@supports not selector(::-webkit-scrollbar)` so they reach Firefox alone; (2) **AntD's own rule** — `.ant-table-wrapper .ant-table` ships `scrollbar-color: rgba(0,0,0,.25) rgba(12,41,112,.07)`, which inherits into `.ant-table-body` and forces the native bar on every table. The fix resets AntD's declaration back to `scrollbar-color: auto` / `scrollbar-width: auto` on `.ant-table`, `.ant-table-body`, `.ant-table-content`, handing the table back to the `::-webkit-scrollbar` path. `::-webkit-scrollbar-button { display: none }` is what removes the arrows.
- **Note on verification**: Playwright/headless runs Linux Chromium, whose native scrollbar renders differently from Windows Chrome, and `getComputedStyle` echoes authored pseudo-element CSS even when the engine ignores it — so automated QA can confirm the **cascade** (which rule wins) but **not** the final Windows rendering. Windows Chrome is the source of truth for this surface.
- **Tokens**: `scrollbarThumb` / `scrollbarThumbHover` flow through the same primitive→semantic→`brandCssVars`→codegen pipeline as every other color; no literals in CSS. The TagsView strip opts out entirely (`scrollbar-width: none`) — it scrolls via wheel/drag, no visible bar.
